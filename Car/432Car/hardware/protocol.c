...
extern _PID1 Distance_PID;
extern _PID2 Turn_PID; 
/**
 * @brief   接收的数据处理
 * @param   void
  ...
  while(1)
  {
    ...
    switch (cmd_type)
    {
      ...
      case SET_P_I_D_CMD:
      {
        ...
        PID1_Param_Input(&Distance_PID,p_temp, i_temp, d_temp);    // 设置 P I D
      }
      break;
      ...
      case SET_TARGET_CMD:
      {
        ...
        PID1_Input_Data(&Distance_PID,actual_temp,15);
      }
      break;
...
//  UART_write(Uart2, (uint8_t *)&set_packet, sizeof(set_packet));    // 发送数据头
//  UART_write(Uart2, (uint8_t *)data, num);                          // 发送参数
//  UART_write(Uart2, (uint8_t *)&sum, sizeof(sum));                  // 发送校验和
}
void Host_pid(uint8_t CH,int Tar,int real)
{
	switch(CH)
    {
		case 1: set_computer_value(SEND_FACT_CMD, CURVES_CH1, &real, 1);break;
		case 2: set_computer_value(SEND_FACT_CMD, CURVES_CH2, &real, 1);break;
		case 3: set_computer_value(SEND_TARGET_CMD, CURVES_CH3, &Tar, 1);set_computer_value(SEND_FACT_CMD, CURVES_CH3, &real, 1);break;
	}
